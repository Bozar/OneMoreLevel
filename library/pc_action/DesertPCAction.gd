...
func _add_hit_point() -> void:
	var pc: Sprite = _ref_DungeonBoard.get_sprite(
			_new_MainGroupTag.ACTOR, _source_position[0], _source_position[1])

	if _ref_ObjectData.get_hit_point(pc) < _new_DesertData.MAX_PC_HP:
		_ref_ObjectData.add_hit_point(pc, _new_DesertData.ADD_PC_HP)
